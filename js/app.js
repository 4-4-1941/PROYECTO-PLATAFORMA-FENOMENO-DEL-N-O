...
(function(){
  ...
  Mapa.init('geo/departamentos.geojson.txt', irDepartamento);
  Oni.init('oni');
  Alerts.init('alerts');
  UI.init();
  ...
  Mapa.onClic(e=>{let hit=null;DISTRITOS.forEach(d=>{if(!hit&&d.latitud!=null&&Math.abs(d.latitud-e.latlng.lat)<0.1&&Math.abs(d.longitud-e.latlng.lng)<0.1)hit=d;});if(hit)irDistrito(hit);});
})();
